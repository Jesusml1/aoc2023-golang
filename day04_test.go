...
func TestScrashCardPart1(t *testing.T) {
	inputTest := []string{
	...
	if totalPoints != expected {
		t.Errorf("Unexpected result: %d", totalPoints)
	}
}

func TestScrashCardPart2(t *testing.T) {
	...
	if totalCards != expected {
		t.Errorf("Unexpected result: %d", totalCards)
	}
}
